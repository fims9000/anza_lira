"""Build flat-cap intervention cases without using SBPP outcomes."""
...
from lira_h1.ribbon import cumulative_arclength, minimal_valid_ribbon_cut
...
class RibbonCase:
    ...
    trace_yx: tuple[tuple[float, float], ...]
...
def split_manifest() -> dict[str, object]:
    ...
    splits = {name: [section for section in range(low, high + 1) if section in available] for name, (low, high) in SPLIT_RANGES.items()}
    values = list(splits.values())
    if any(set(values[i]) & set(values[j]) for i in range(len(values)) for j in range(i + 1, len(values))):
        raise AssertionError("H1 sections overlap")
    payload = {
        "splits": splits,
        "ranges": {name: list(bounds) for name, bounds in SPLIT_RANGES.items()},
        "frozen_before_section_347_access": True,
        "h1_confirm_hash_only": True,
...
def _interval(trace: CrowdTrace, split: str, gap_length: int) -> tuple[np.ndarray, int, int, np.ndarray] | None:
    ...
    feasible = np.flatnonzero((arc >= context) & (arc <= arc[-1] - context - gap_length))
    if not len(feasible):
        return None
    fraction = (_stable_int(trace.trace_id, split, gap_length, "position") % 10001) / 10000.0
    start = int(feasible[min(len(feasible) - 1, int(fraction * len(feasible)))])
    end = int(np.searchsorted(arc, arc[start] + gap_length, side="left"))
    if end >= len(points) - context or start < context or end <= start:
        return None
    return points, start, end, arc
...
def _placement_result(trace: CrowdTrace, split: str, gap_length: int, probability: np.ndarray, other_trace_mask: np.ndarray) -> tuple[dict[str, object], RibbonCase | None]:
    ...
    points, start, end, arc = interval
    hidden = points[start : end + 1]
    margin = int(PROTOCOL["placement"]["image_border_margin_px"])
    if hidden[:, 0].min() < margin or hidden[:, 1].min() < margin or hidden[:, 0].max() >= probability.shape[0] - margin or hidden[:, 1].max() >= probability.shape[1] - margin:
    ...
    cut, ribbon, cut_support = minimal_valid_ribbon_cut(
        probability, points, float(arc[start]), float(arc[end]), left_anchor, right_anchor,
        left_context, right_context, other_trace_mask,
    )
    eligible = int(cut.status not in ("INELIGIBLE_ANCHOR_SUPPORT", "INELIGIBLE_PRE_DISCONNECTED"))
    row = {
        **base, "start_index": start, "end_index": end, "status": cut.status, "eligible_before_treatment": eligible,
        "s_a": float(arc[start]), "s_b": float(arc[end]), "radius_px": "" if cut.radius is None else cut.radius,
        "collateral_fraction": cut.collateral_fraction, "pre_connected": int(cut.pre_connected),
        "post_connected": "" if cut.post_connected is None else int(cut.post_connected),
        "left_context_supported": cut.left_context_supported, "right_context_supported": cut.right_context_supported,
        "ribbon_pixels": cut.ribbon_pixels,
    }
    if cut.status != "VALID" or ribbon is None or cut_support is None or cut.radius is None:
        return row, None
    ...
    case = RibbonCase(
        case_id, split, trace.section_id, trace.annotator, trace.trace_id, gap_length, start, end,
        float(arc[start]), float(arc[end]), cut.radius, cut.collateral_fraction, source[0], source[1],
        _serialize(points), _serialize(hidden), _serialize(left_anchor), _serialize(right_anchor), _serialize(destination),
    )
    row["case_id"] = case_id
    return row, case
...
def save_cases(path: Path, cases: list[RibbonCase]) -> None:
    rows = []
    encoded = ("source_yx", "source_tangent_yx", "trace_yx", "hidden_yx", "left_anchor_yx", "right_anchor_yx", "destination_context_yx")
    for case in cases:
        row = asdict(case)
        for key in encoded:
            row[key] = json.dumps(row[key], separators=(",", ":"))
        rows.append(row)
    _write_csv(path, rows, [field.name for field in dataclass_fields(RibbonCase)])


def load_cases(path: Path) -> tuple[RibbonCase, ...]:
    output = []
    with path.open() as handle:
        for value in csv.DictReader(handle):
            output.append(RibbonCase(
                ...
                gap_length_px=int(value["gap_length_px"]), start_index=int(value["start_index"]), end_index=int(value["end_index"]), s_a=float(value["s_a"]), s_b=float(value["s_b"]),
                radius_px=int(value["radius_px"]), collateral_fraction=float(value["collateral_fraction"]), source_yx=tuple(json.loads(value["source_yx"])), source_tangent_yx=tuple(json.loads(value["source_tangent_yx"])),
                trace_yx=tuple(map(tuple, json.loads(value["trace_yx"]))), hidden_yx=tuple(map(tuple, json.loads(value["hidden_yx"]))), left_anchor_yx=tuple(map(tuple, json.loads(value["left_anchor_yx"]))),
                right_anchor_yx=tuple(map(tuple, json.loads(value["right_anchor_yx"]))), destination_context_yx=tuple(map(tuple, json.loads(value["destination_context_yx"]))),
            ))
    return tuple(output)


def recover_split(split: str, manifest: dict[str, object]) -> dict[tuple[int, str], tuple[CrowdTrace, ...]]:
    if split in ("h1_buffer", "h1_confirm"):
        raise PermissionError(f"{split} contents are hash-only")
    annotators = TRAIN_ANNOTATORS if split == "h1_train" else HELDOUT_ANNOTATORS
    _report, recovered = audit_trace_identity(ROOT / "data/cracks/annotations", list(manifest["splits"][split]), tuple(annotators))
    return recovered


def build_split(split: str, recovered: dict[tuple[int, str], tuple[CrowdTrace, ...]], output: Path, dense_cache: Path) -> dict[str, object]:
    rows: list[dict[str, object]] = []
    cases: list[RibbonCase] = []
    length_counts = Counter({length: 0 for length in GAP_LENGTHS})
    groups = sorted(recovered.items())
    processed = 0
    for (section_id, _annotator), local in groups:
        ...
        for trace, own in zip(local, dilated):
            arc_length = float(cumulative_arclength(np.asarray(trace.points_yx, dtype=np.float64))[-1])
            feasible = [length for length in GAP_LENGTHS if arc_length >= length + 2 * int(PROTOCOL["placement"]["minimum_context_px_each_side"])]
            if not feasible:
                rows.append({"split": split, "section_id": section_id, "annotator": trace.annotator, "trace_id": trace.trace_id, "gap_length_px": "", "status": "INELIGIBLE_INTERVAL", "eligible_before_treatment": 0})
                continue
            ...
            if case is not None:
                cases.append(case)
        processed += len(local)
        if processed and (processed % 500 < len(local) or (section_id, _annotator) == groups[-1][0]):
            print(f"phase=H1_RIBBON split={split} traces={processed}", flush=True)
    _write_csv(output / "eligibility.csv", rows)
    save_cases(output / "cases.csv", cases)
    ...
    summary = {
        "split": split, "reliable_traces": len(rows), "eligible_before_treatment": eligible, "valid_cases": len(cases),
        "retention": len(cases) / eligible if eligible else 0.0, "treatment_validity": 1.0 if cases else None,
        "status_counts": dict(sorted(status_counts.items())),
        "minimal_disconnect_radius_counts": {str(radius): minimal_radii[radius] for radius in CUT_RADII},
        "median_selected_radius": float(np.median([case.radius_px for case in cases])) if cases else None,
        "gap_length_counts": {str(length): sum(case.gap_length_px == length for case in cases) for length in GAP_LENGTHS},
        "image_changed": False, "candidate_used_for_filtering": False,
    }
    radius_rows = [{"radius_px": radius, "minimal_disconnect_count": minimal_radii[radius], "valid_count": sum(case.radius_px == radius for case in cases)} for radius in CUT_RADII]
    _write_csv(output / "radius_distribution.csv", radius_rows)
    return summary
